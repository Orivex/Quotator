...
const Index = () => {
  ...
  useEffect(() => {
    ...
    const loadRandomQuote = async () => {
      ...
      if(nRows == 0) {
        setRandomQuote({quote: 'This could be your quote. Add a quote and reload to see your first quote!', author: 'Unknown'})
      }
      else if (nRows == 1) {
        result = await db.getAllAsync(`SELECT id, quote, author FROM quotes WHERE neverShow = 0`);
        if(result.length == 0) {
          setRandomQuote({quote: 'The only quote that you have added is set to never be shown on the home screen.', author: 'Unknown'})
        }
        else {
        ...
        if(result.length > 0) {
          ...
          if(neverShow_nRows == nRows) {
            setRandomQuote({quote: 'You have set all quotes to not be shown on the home screen.', author: 'Unknown'})
          }
        }
      }
    };
    ...
    <View style={styles.container}>
      ...
      <View style={styles.quoteSection}>
        ...
        <View style = {styles.quoteContainer}>
          <Text style = {styles.quoteText} adjustsFontSizeToFit={true} numberOfLines={10}>
            "{randomQuote.quote}"
          </Text>
